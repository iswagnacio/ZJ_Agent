...
## segment
- **Segment_ROI_API_v10** — 从输入图片生成 ROI（细胞/颗粒等目标的轮廓）。当目标形状需要被分割、计数或后续测量时使用；按形状特性选 cellpose（圆形/核）或 threshold（强度阈值）。  `POST /api/v10/segment-roi/run`  · methods: cellpose, threshold
...
## calculate
- **Formula_API_v10** — 当需要产出最终报表/答案——对最终 target 的计数、比例、OD/IOD 等计算量或比值——时使用。  `POST /api/v10/formula/run`
